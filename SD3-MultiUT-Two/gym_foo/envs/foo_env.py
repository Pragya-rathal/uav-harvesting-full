...
class FooEnv(gym.Env):
    ...
    def __init__(self, LoadData=True, Train=False, multiUT=True, Trajectory_mode='Kmeans', MaxStep=41, RicianK=5.0, trajectory_mode=None, observation_mode='legacy', reward_mode='legacy', csi_mode='perfect', sigma_csi=0.05, flight_lambda=1e-3, max_step_xy=10.0, max_step_z=4.0, num_users=None):
        globe._init()
        ...
        #number of user 
        globe.set_value('N_u', int(os.getenv("UAV_NUM_USERS", num_users if num_users is not None else 3)))
        ...
        if LoadData == True:
            env_num_users = os.getenv("UAV_NUM_USERS")
            if env_num_users is not None:
                user_count = max(int(env_num_users), 1)
            elif num_users is not None:
                user_count = max(int(num_users), 1)
            else:
                user_count = 3

            split_name = "Train" if Train else "Test"
            self.num_users = user_count

            if multiUT == True:
                for idx in range(user_count):
                    ut_path = f"../CreateData/{split_name}_Trajectory_UT_{idx}.csv"
                    if not os.path.exists(ut_path):
                        raise FileNotFoundError(
                            f"Missing user trajectory file: {ut_path}. "
                            "Run publication_suite/generate_user_trajectories.py first."
                        )
                    globe.set_value(f'UT_{idx}', np.loadtxt(ut_path, delimiter=","))

                traj_path = f"../CreateData/{Trajectory_mode}_{split_name}_Trajectory_{user_count}.csv"
                if not os.path.exists(traj_path):
                    legacy_user_count = 3 if user_count > 1 else 1
                    legacy_path = f"../CreateData/{Trajectory_mode}_{split_name}_Trajectory_{legacy_user_count}.csv"
                    if os.path.exists(legacy_path):
                        traj_path = legacy_path
                if not os.path.exists(traj_path):
                    raise FileNotFoundError(
                        f"Missing UAV trajectory file: {traj_path}. "
                        "Run publication_suite/generate_user_trajectories.py first."
                    )
                globe.set_value('UAV_Trajectory', np.loadtxt(traj_path, delimiter=","))
            else:
                self.num_users = 1
                ut_path = f"../CreateData/{split_name}_Trajectory_UT_0.csv"
                if not os.path.exists(ut_path):
                    raise FileNotFoundError(
                        f"Missing user trajectory file: {ut_path}. "
                        "Run publication_suite/generate_user_trajectories.py first."
                    )
                globe.set_value('UT_0', np.loadtxt(ut_path, delimiter=","))
                traj_path = f"../CreateData/{Trajectory_mode}_{split_name}_Trajectory_1.csv"
                if not os.path.exists(traj_path):
                    raise FileNotFoundError(
                        f"Missing UAV trajectory file: {traj_path}. "
                        "Run publication_suite/generate_user_trajectories.py first."
        ...
        self.action_space = spaces.Box(0, 1, shape=(36 + (3 if learned else 0), ), dtype=np.float32)
        if self.observation_mode == "enhanced":
            self.observation_space = spaces.Box(-1e6, 1e6, shape=(self._enhanced_obs_dim, ), dtype=np.float32)
        else:
            self.observation_space = spaces.Box(0, 1e6, shape=(2, ), dtype=np.float32)
    ...
    def step(self, action):
        ...
        power_2 = mt.pow(10, ((action_main[2]-1)*30/10+3))
        power_3 = mt.pow(10, ((action_main[3]-1)*30/10+3))
        Theta_R = action_main[4: 4 + globe.get_value('RIS_L')] * 2 * np.pi
        Omega_R = np.abs(np.around(action_main[4 + globe.get_value('RIS_L'):]))
        reward_args = (tau, power_1, power_2, power_3, Theta_R, Omega_R)
    ...
    def reset(self, *, seed=None, options=None):
        ...
        UT_1 = globe.get_value('UT_1')[step_idx]
        UT_2 = globe.get_value('UT_2')[step_idx]
        ...
        radio_state = np.array([distance_AP_RIS, distance_RIS_UT_0, distance_RIS_UT_1, distance_RIS_UT_2], dtype=np.float32)
    ...
    def capacity (self, tau, power_1, power_2, power_3, Theta_R, L_U, L_AP, UT_0, UT_1, UT_2, Omega_R):
        ...
        UT_link_2 = self._csi_perturb(1*np.linalg.multi_dot([G, coefficients, h_ru]))
        ...
        UT_link_3 = self._csi_perturb(1*np.linalg.multi_dot([G, coefficients, h_ru]))
